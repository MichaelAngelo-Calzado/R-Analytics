...
adjust_length <- function(short_vec, target_length) {
  if (length(short_vec) < target_length) {
    return(c(short_vec, rep(NA, target_length - length(short_vec))))
  }
  return(short_vec)
}

for (i in seq_along(urls3)) {
  session <- bow(urls3[i], user_agent = "Educational")
  webpage <- scrape(session)

  nam3 <- webpage %>% html_nodes(".a-profile-name") %>% html_text()
  ...
  filtered_names <- c()
  for (name in nam3) {
    if (name == "Amazon Customer" && non_amazon_seen) {
      filtered_names <- c(filtered_names, name)
    } else if (name != "Amazon Customer") {
      filtered_names <- c(filtered_names, name)
      non_amazon_seen <- TRUE
    }
  }
  names3[[i]] <- filtered_names[!duplicated(filtered_names) | filtered_names == "Amazon Customer"]

  product_name <- webpage %>%
    html_nodes('.a-size-large.product-title-word-break') %>%
    html_text() %>%
    rep(length.out = length(names3[[i]]))
  nameofproduct3[[i]] <- product_name

  rate3 <- webpage %>% html_nodes(".a-icon-alt") %>% html_text()
  rate3 <- rate3[!grepl("Previous page|Next page|Previous set of slides|Next set of slides", rate3, ignore.case = TRUE)]
  ratings <- gsub(" out of 5 stars", "", rate3)
  ratings3[[i]] <- adjust_length(ratings, length(names3[[i]]))
  ...
  title3[[i]] <- webpage %>%
    html_nodes(".a-size-base.review-title.a-color-base.review-title-content.a-text-bold") %>%
    html_text() %>%
    trimws()
...
```{r}
cate3 <- c("Category 1: WATCH", "Category 2: SKINCARE", "Category 3: PANTS", "Category 4", "Category 5")
...
productnumber2 <- vector("list", length(productnumbe2))
for (i in seq_along(productnumbe2)) {
    productnumber2[[i]] <- productnumbe3[i]
}
for (i in seq_along(urls2)) {
    productnumber2[[i]] <- rep(productnumber3[[i]], length.out = length(names2[[i]]))
}
```