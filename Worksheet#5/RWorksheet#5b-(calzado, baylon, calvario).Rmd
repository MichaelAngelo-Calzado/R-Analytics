...
```
```{r}
urls2b <- c('https://www.amazon.com/Amazon-Essentials-Slim-Fit-Stretch-Black/dp/B07BJKTXX3/ref=sr_1_1_ffob_sspa?crid=27TSZ91XTCRLW&dib=eyJ2IjoiMSJ9.lxTaNXjcDhEwKCXa5-a-BYVs1xuBogfsFXjGr21eELTn-kCQqhVS37mhEpiq1V30lPN2Nfn3b18GfyLXbY4K5rdG_iAcI6uDZ6-uuySkYD2owEBW5Mpj4wvMjZm99wDcJEeEEjSs76L0BamJLfkfmF48qFhaz1j2q8BuJs9zxjIY0inNy_Va296tTcK-MipEAzumwTOWlAoVvYYllskGuILEXwm8IwkWJZDhYKhPMpdHFia1EvTXgnfCE_IB2r0hqmm6SWDQi48n1-gWkn_8Wp7ElOGqSkvjAY-xfOvdAiY.gq-wH65kx5Udvj3-j5xR_blV-FIoERShoIiXZzK5X3c&dib_tag=se&keywords=pants&qid=1733754251&sprefix=pants%2Caps%2C468&sr=8-1-spons&sp_csd=d2lkZ2V0TmFtZT1zcF9hdGY&psc=1')
nameofproduct2b <- vector("list", length(urls2b))
names2b <- vector("list", length(urls2b))
ratings2b <- vector("list", length(urls2b))
dates2b <- vector("list", length(urls2b))
...
```{r}
for (i in seq_along(cate3)) {
    category3[[i]] <- rep(category3[[i]], length.out = length(names2[[i]]))
}
```
```{r}
productnumbe2 <- c("Product 1", "Product 2", "Product 3", "Product 4", "Product 5", "Product 6", "Product 7", "Product 8", "Product 9", "Product 10")
