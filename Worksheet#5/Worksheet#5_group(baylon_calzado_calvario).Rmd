...
1. Each group needs to extract the top 50 tv shows in Imdb.com
```{r}
library(rvest)
library(dplyr)
library(stringr)
...
```{r}
yea = page %>% html_nodes(".sc-300a8231-7.eaXxft.cli-title-metadata-item") %>% html_text()
...
```{r}
episode <- page %>% html_nodes(".sc-300a8231-7.eaXxft.cli-title-metadata-item") %>%
html_text()
...
```
2. From the 50tv shows, select at least 5 tv shows to scrape 20 user reviews
```{r}
...
review_data_frames <- vector("list", length(rl))
```
...
write.csv(BreakingBad, "Breaking Bad reviews.csv")
...
write.csv(PlanetEarthII, "PlanetEarthII reviews.csv")
...
```
```{r}
TextReview2 <- text_review[[2]]
head(TextReview2, 20)
```

```{r}
PlanetEarth <- data.frame(NAME[[3]],
    Date = date_of_review[[3]],
    Rating = user_ratings[[3]],
    Title = title_of_review[[3]],
    stringsAsFactors = FALSE
  )
write.csv(PlanetEarth, "PlanetEarth reviews.csv")
print(head(PlanetEarth))
PlanetEarth %>%
  kable("latex", booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")
```
```{r}
TextReview3 <- text_review[[3]]
head(TextReview3, 20)
```

```{r}
BondOfBrothers <- data.frame(NAME[[4]],
    Date = date_of_review[[4]],
    Rating = user_ratings[[4]],
    Title = title_of_review[[4]],
    stringsAsFactors = FALSE
  )
write.csv(BondOfBrothers, "BondOfBrothers reviews.csv")
print(head(BondOfBrothers))
BondOfBrothers %>%
  kable("latex", booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")
```
```{r}
TextReview4 <- text_review[[4]]
head(TextReview4, 20)
```

```{r}
Chernobyl <- data.frame(NAME[[5]],
    Date = date_of_review[[5]],
    Rating = user_ratings[[5]],
    Title = title_of_review[[5]],
    stringsAsFactors = FALSE
  )
write.csv(Chernobyl, "Chernobyl reviews.csv")
print(head(Chernobyl))
Chernobyl %>%
  kable("latex", booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")
```
```{r}
TextReview5 <- text_review[[5]]
head(TextReview5, 20)
```
3. Create a time series graph for the tv shows released by year.
```{r}
